...
object PatternMatching:
   ...
   * What if no cases match? MatchError What's the type returned? The lowest
   * common ancestor of all types on the RHS of each branch. */
  ...
  /**
   * Exercise show(Sum(Number(2), Number(3))) = "2 + 3" show(Sum(Sum(Number(2),
   * Number(3)), Number(4)) = "2 + 3 + 4" show(Prod(Sum(Number(2), Number(3)),
   * Number(4))) = "(2 + 3) * 4" show(Sum(Prod(Number(2), Number(3)), Number(4)) =
   * "2 * 3 + 4"
   */
  sealed trait Expr
