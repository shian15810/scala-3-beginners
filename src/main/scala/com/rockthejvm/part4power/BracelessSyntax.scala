...
object BracelessSyntax:
  ...
  // java-style
  val anIfExpression_v2 = if 2 > 3 then "bigger" else "smaller"

  // compact
  val anIfExpression_v3 = if 2 > 3 then "bigger" else "smaller"

  // scala 3
  val anIfExpression_v4 =
    if 2 > 3 then "bigger" // higher indentation than the if part
    else "smaller"
  ...
  /* class definition with significant indentation (same for traits, objects,
   * enums etc) */
  ...
  def main(args: Array[String]): Unit = println(computeMeaningOfLife(78))
end BracelessSyntax
