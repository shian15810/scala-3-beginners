...
object Options:
  ...
  val stringLength =
    ...
    if potentialString == null then -1 else potentialString.length
   ...
   * connection, print "Conn successful" or "Conn failed"
   */
  ...
  object Connection:
    ...
    def apply(host: String, port: String): Option[Connection] =
      if random.nextBoolean() then Some(new Connection) else None

  // defensive style (in an imperative language e.g. Java)
  /* String host = config("host") String port = config("port") if (host != null)
   * if (port != null) Connection conn = Connection.apply(host, port) if (conn
   * != null) return conn.connect() // ... that's just the happy path, we need
   * to add the rest of the branches */
  ...
  // compact
  val connStatus_v2 = config.get("host")
