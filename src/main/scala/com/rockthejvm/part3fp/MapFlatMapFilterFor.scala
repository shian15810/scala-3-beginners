...
object MapFlatMapFilterFor:
  ...
  /* All the possible combinations of all the elements of those lists, in the
   * format "1a - black" */
  val numbers = List(1, 2, 3, 4)
  val chars   = List('a', 'b', 'c', 'd')
  val colors  = List("black", "white", "red")

  /* lambda = num => chars.map(char => s"$num$char") [1,2,3,4].flatMap(lambda) =
   * ["1a", "1b", "1c", "1d", "2a", "2b", "2c", "2d", ...] lambda(1) =
   * chars.map(char => s"1$char") = ["1a", "1b", "1c", "1d"] lambda(2) = .. =
   * ["2a", "2b", "2c", "2d"] lambda(3) = ..
   * lambda(4) = .. */
  val combinations = numbers.withFilter(_ % 2 == 0).flatMap(
    number => chars.flatMap(char => colors.map(color => s"$number$char - $color")))
  ...
  def main(args: Array[String]): Unit =
    ...
    for num <- numbers do println(num)
