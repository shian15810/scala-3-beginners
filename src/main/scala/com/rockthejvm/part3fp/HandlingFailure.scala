...
object HandlingFailure:
  ...
  val stringLengthDefensive =
    try
      ...
      aString.length
    catch case e: RuntimeException => -1
  ...
  /**
   * Exercise: obtain a connection, then fetch the url, then print the resulting
   * HTML
   */
  val host         = "localhost"
  val port         = "8081"
  ...
  val finalHtml =
    try
      ...
      val html =
        try conn.get(myDesiredURL)
        catch case e: RuntimeException => s"<html>${e.getMessage}</html>"
    catch case e: RuntimeException => s"<html>${e.getMessage}</html>"
  ...
  val maybeHtml_v2 =
    ...
    yield html
  val finalResult_v2 = maybeHtml
    .fold(e => s"<html>${e.getMessage}</html>", s => s)

  def main(args: Array[String]): Unit = println(finalResult)
end HandlingFailure
