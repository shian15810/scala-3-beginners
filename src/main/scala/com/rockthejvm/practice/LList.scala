...
case class Empty[A]() extends LList[A]:
  ...
  override def zipWith[B, T](list: LList[T], zip: (A, T) => B) =
    if !list.isEmpty then
      throw new IllegalArgumentException("Zipping lists of nonequal length")
    else Empty()
...
case class Cons[A](override val head: A, override val tail: LList[A])
  ...
  /* example [1,2,3] ++ [4,5,6] new Cons(1, [2,3] ++ [4,5,6]) =
  ...
  override def sort(compare: (A, A) => Int) =
    /* compare = x - y insert(3, [1,2,4]) =
     * Cons(1, insert(3, [2,4])) =
     * Cons(1, Cons(2, insert(3, [4]))) =
     * Cons(1, Cons(2, Cons(3, [4]))) = [1,2,3,4] */
    // insertion sort, O(n^2), stack recursive
    def insert(elem: A, sortedList: LList[A]): LList[A] =
      if sortedList.isEmpty then Cons(elem, Empty())
      else if compare(elem, sortedList.head) <= 0 then Cons(elem, sortedList)
   ...
   * = [].foldLeft(10)(x + y)
   * = 10 */
  override def foldLeft[B](start: B)(operator: (B, A) => B) = tail
    .foldLeft(operator(start, head))(operator)
end Cons
...
object LList:
  def find[A](list: LList[A], predicate: A => Boolean): A =
    ...
    else if predicate(list.head) then list.head
...
object LListTest:
  def main(args: Array[String]): Unit =
    ...
    val doublerList = new Function1[Int, LList[Int]]:
      override def apply(value: Int) = Cons(value, Cons(value + 1, Empty()))
    ...
    val numbersNested_v2 = first3Numbers
      .map(value => Cons(value, Cons(value + 1, Empty())))
    ...
    val flattenedList_v2 = first3Numbers
      .flatMap(value => Cons(value, Cons(value + 1, Empty())))
    ...
    val zippedList = first3Numbers.zipWith[String, String](
      someStrings,
      (number, string) => s"$number-$string")
    println(zippedList)
...
class DoublerList extends Transformer[Int, LList[Int]]:
  override def transform(value: Int) = Cons(value, Cons(value + 1, Empty()))
