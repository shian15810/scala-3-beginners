...
object Inheritance:
  ...
  class Crocodile extends Animal:
    ...
    def eat(person: Person): Unit =
      println(s"I'm eating a human with the name ${person.name}")
    def eat(person: Person, dog: Dog): Unit =
      println("I'm eating a human AND the dog")
    // def eat(): Int = 45 // not a valid overload
    def eat(dog: Dog, person: Person): Unit =
      println("I'm eating a human AND the dog")
  end Crocodile

  def main(args: Array[String]): Unit = println(dog) // println(dog.toString)
end Inheritance
