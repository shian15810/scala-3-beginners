 ...
 *   - print()
 *
 * Benefits: + well in distributed environments + easier to read and understand
 * code
 */
...
class Counter(count: Int = 0):
  ...
  def decrement(): Counter = if count == 0 then this else new Counter(count - 1)

  def increment(n: Int): Counter =
    if n <= 0 then this else increment().increment(n - 1) // vulnerable to SOs

  def decrement(n: Int): Counter =
    if n <= 0 then this else decrement().decrement(n - 1)
