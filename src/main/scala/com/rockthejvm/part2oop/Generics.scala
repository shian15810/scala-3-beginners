...
object Generics:
  ...
  class NonEmptyIntList(override val head: Int, override val tail: IntList)
    extends IntList
  ...
  class NonEmptyStringList(
      ...
      override val tail: StringList)
    extends StringList
  // ... and so on for all the types you want to support
  /* Pros:
   * - keeps type safety: you know which list holds which kind of elements Cons:
   * - boilerplate
   * - unsustainable
   * - copy/paste... really? */
  ...
  class EmptyGeneralList extends GeneralList:
    override def head = throw new NoSuchElementException
    override def tail = throw new NoSuchElementException
  class NonEmptyGeneralList(
      override val head: Any,
      override val tail: GeneralList)
    extends GeneralList
  /* Pros:
   * - no more code duplication
   * - can support any type Cons:
   * - lost type safety: can make no assumptions about any element or method
   * - can now be heterogeneous: can hold cats and dogs in the same list (not
   * funny) */
